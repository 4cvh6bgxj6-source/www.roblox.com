...
Roblox.BundleDetector = (function () {
  ...
  return {
    ...
    getCdnProviderAndReportMetrics: function (
      bundleUrl,
      bundleName,
      loadState,
      bundleContentType
    ...
    logToEphemeralCounter: function (ephemeralCounterName) {
      ...
      xhr.open(
        "POST",
        metricsApiUrl + "/v1/games/report-event?name=" + ephemeralCounterName,
        true
      );
      xhr.withCredentials = true;
      xhr.send();
        ...
        xhr.onreadystatechange = function () {
          if (xhr.readyState === xhr.HEADERS_RECEIVED) {
            ...
            if (cdnProvider && cdnProvider.length > 0) {
              ...
              Roblox.BundleDetector.logToEphemeralCounter(
                counter.unknown + "_" + fileType
              );
            }
          } else if (xhr.readyState === xhr.DONE) {
            // append status to cdn provider so we know its not related to network error.
            Roblox.BundleDetector.logToEventStream(
              failedBundle,
              ctx,
              cdnProvider,
              xhr.status
    ...
    verifyBundles: function (document) {
      ...
      for (var i = 0; i < scripts.length; i++) {
        ...
        if (item.src && monitor && bundleName) {
          if (
            !Roblox.BundleDetector.jsBundlesLoaded.hasOwnProperty(bundleName)
          ) {
